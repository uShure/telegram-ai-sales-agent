...
class CoursesDatabase {
  ...
  async getCourseByName(name: string): Promise<Course | null> {
    try {
      ...
      const course = this.db.prepare(query).get(`%${name}%`) as Course | undefined;
      return course || null;
    } catch (err) {
  ...
  async getCoursesByPriceRange(minPrice: number, maxPrice: number): Promise<Course[]> {
    try {
      const query = `
        SELECT * FROM courses
      ...
      console.log(`✅ Найдено курсов в диапазоне ${minPrice}-${maxPrice}: ${rows.length}`);
      return rows;
    } catch (err) {
  ...
  async searchCourses(keywords: string) {
    console.log(`🔍 Поиск курсов по ключевым словам: "${keywords}"`);
    ...
    try {
      ...
      const rows = stmt.all(...patterns, ...patterns, ...patterns, ...patterns);
      if (rows.length > 0) {
