...
import type { TelegramBot } from "../telegram/telegramClient";
...
export class AdminPanel {
  ...
  async listRecentConversations(limit = 10): Promise<void> {
    try {
      const conversations = await database.getRecentConversations(limit);
  ...
  async analyzeConversation(userId: string): Promise<void> {
    try {
      ...
      console.log(
        `📅 Начало диалога: ${conversation.createdAt.toLocaleString("ru-RU")}`,
      );
      console.log(
        `📅 Последнее сообщение: ${conversation.lastMessageAt.toLocaleString("ru-RU")}`,
      );
      console.log(`💬 Всего сообщений: ${conversation.messages.length}`);
      console.log(
        `📊 Статус: ${this.getStatusText(conversation.clientStatus)}`,
      );

      console.log("\n🤖 AI-анализ диалога...");
      const insights =
        await this.aiAgent.analyzeConversationForInsights(conversation);
