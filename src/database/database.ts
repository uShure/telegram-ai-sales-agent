...
export interface Conversation {
  ...
  userId: string;
  userName: string;
  userUsername?: string;  // Telegram username для связи
  userPhone?: string;
  messages: Message[];
  lastMessageAt: Date;
...
class Database {
  ...
  async initialize(): Promise<void> {
    ...
    // Create indexes
    await this.run('CREATE INDEX IF NOT EXISTS idx_conversations_userId ON conversations(userId)');
    await this.run('CREATE INDEX IF NOT EXISTS idx_messages_conversationId ON messages(conversationId)');
    await this.run('CREATE INDEX IF NOT EXISTS idx_follow_ups_scheduledAt ON follow_ups(scheduledAt)');
  }

  async getOrCreateConversation(userId: string, userName: string, userPhone?: string): Promise<number> {
    // Check if conversation exists
    const existing = await this.get('SELECT id FROM conversations WHERE userId = ?', [userId]);
  ...
  async addMessage(conversationId: number, role: 'user' | 'assistant', content: string): Promise<void> {
    await this.run(
  ...
  async getFullConversation(userId: string): Promise<Conversation | null> {
    const conv = await this.get('SELECT * FROM conversations WHERE userId = ?', [userId]);
    if (!conv) return null;
    ...
    return {
      ...
      followUpScheduled: conv.followUpScheduled ? new Date(conv.followUpScheduled) : undefined,
      createdAt: new Date(conv.createdAt),
      updatedAt: new Date(conv.updatedAt)
    };
  }

  async updateClientStatus(conversationId: number, status: Conversation['clientStatus']): Promise<void> {
    await this.run(
  ...
  async scheduleFollowUp(conversationId: number, scheduledAt: Date, message?: string): Promise<void> {
    await this.run(
  ...
  async getPendingFollowUps(): Promise<Array<{followUp: FollowUp, conversation: Conversation}>> {
  ...
  async markFollowUpCompleted(followUpId: number): Promise<void> {
    await this.run('UPDATE follow_ups SET completed = 1 WHERE id = ?', [followUpId]);
  }
