...
export class EnhancedTelegramBot {
  ...
  constructor() {
    ...
    if (!apiId || !apiHash) {
      throw new Error('API_ID и API_HASH должны быть установлены в .env файле');
    }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ База курсов подключена: ${coursesCount.length} активных курсов`);
    } catch (error) {
  ...
  private async resolveEntity(message: any, senderId: string): Promise<{
    entity: any | null;
    userName: string;
    userPhone: string;
  }> {
    ...
    if (!entity) {
      const methods = [
        ...
        async () => await this.client.getEntity(parseInt(senderId)),
  ...
  private async handleNewMessage(event: any): Promise<void> {
    try {
      ...
      const { entity, userName, userPhone } = await this.resolveEntity(message, senderId);

      console.log(`📨 Новое сообщение от ${userName} (ID: ${senderId}): ${message.text}`);
      ...
      const aiResponse = await this.aiAgent.generateResponse(
        conversation,
        message.text,
        this.client  // Передаем TelegramClient для эскалации
      );

      await database.updateClientStatus(conversationId, aiResponse.clientStatus);
      if (aiResponse.suggestedProducts.length > 0) {
        await database.updateInterestedProducts(conversationId, aiResponse.suggestedProducts);
      }

      if (aiResponse.shouldScheduleFollowUp && !conversation.followUpScheduled) {
        const followUpDate = new Date();
        followUpDate.setDate(followUpDate.getDate() + parseInt(process.env.FOLLOW_UP_DAYS || '3'));
        await database.scheduleFollowUp(conversationId, followUpDate);
        console.log(`📅 Запланирован follow-up на ${followUpDate.toLocaleDateString()}`);
      }
      ...
      if (sent) {
        await database.addMessage(conversationId, 'assistant', aiResponse.response);
        ...
        await database.addMessage(conversationId, 'assistant', `[НЕ ДОСТАВЛЕНО] ${aiResponse.response}`);
    ...
    setInterval(async () => {
      try {
        ...
        for (const { followUp, conversation } of pendingFollowUps) {
          try {
            const fullConversation = await database.getFullConversation(conversation.userId);
            if (!fullConversation) continue;

            const followUpMessage = await this.aiAgent.generateFollowUpMessage(fullConversation);
            ...
            if (sent) {
              await database.addMessage(conversation.id!, 'assistant', followUpMessage);
              await database.markFollowUpCompleted(followUp.id!);
              console.log(`✅ Follow-up отправлен для ${conversation.userName}`);
            }

          } catch (error) {
            console.error(`❌ Ошибка follow-up для ${conversation.userName}:`, error);
          }
        }
      } catch (error) {
  ...
  async sendProactiveMessage(userId: string, message: string): Promise<void> {
    try {
      ...
      if (sent) {
        console.log(`📤 Проактивное сообщение отправлено пользователю ${userId}`);
      } else {
        console.log(`❌ Не удалось отправить проактивное сообщение пользователю ${userId}`);
      }
    } catch (error) {
