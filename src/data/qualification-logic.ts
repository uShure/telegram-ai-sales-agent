...
export function analyzeQualification(messages: string[]): ClientQualification {
  ...
  // Проверяем формат
  if (allText.includes('очн') || allText.includes('офлайн') || allText.includes('офлайн') ||
      allText.includes('творческ') || allText.includes('деревн')) {
    qualification.format = 'offline';
  } else if (allText.includes('онлайн') || allText.includes('дистанц') || allText.includes('удален')) {
    qualification.format = 'online';
  }

  // Проверяем направление
  if (allText.includes('стрижк') || allText.includes('парикмахер') || allText.includes('стричь')) {
    qualification.direction = 'haircuts';
  } else if (allText.includes('колор') || allText.includes('окраш') || allText.includes('цвет') ||
             allText.includes('блонд') || allText.includes('днк')) {
    qualification.direction = 'coloring';
  } else if (allText.includes('салон') || allText.includes('управлен') || allText.includes('бизнес') ||
             allText.includes('руководител')) {
    qualification.direction = 'business';
  }

  // Проверяем опыт
  if (allText.includes('начина') || allText.includes('новичок') || allText.includes('с нуля') ||
      allText.includes('нет опыта')) {
    qualification.experience = 'beginner';
  } else if (allText.includes('опыт') || allText.includes('работаю') || allText.includes('лет в профессии') ||
             allText.includes('года в профессии')) {
    qualification.experience = 'intermediate';
  } else if (allText.includes('эксперт') || allText.includes('преподава') || allText.includes('наставник') ||
...
export function getQualificationQuestions(qualification: ClientQualification): string {
  const questions: string[] = [];

  // Приветствие если это первое сообщение
  if (!qualification.format && !qualification.direction && !qualification.experience) {
    questions.push('Здравствуйте! Меня зовут Вера, я консультант академии SOINTERA. Помогу подобрать идеальный курс именно для вас! 😊');
  }

  // Порядок важен: сначала формат, потом направление, потом опыт
  if (!qualification.format) {
    questions.push('Сначала определимся с форматом: вам удобнее учиться очно в Творческой деревне или онлайн?');
  } else if (!qualification.direction) {
    questions.push('Отлично! Теперь выберите направление: стрижки или колористика?');
  } else if (!qualification.experience) {
    questions.push('Супер! И последний вопрос: какой у вас опыт работы? Это поможет подобрать программу нужного уровня.');
  }
...
export function isGeneralRequest(message: string): boolean {
  ...
  if (exactPhrases.some(phrase => lower === phrase || lower === phrase + '?')) {
    return true;
  }
  ...
  if (message.length < 40 && generalPhrases.some(phrase => lower.includes(phrase))) {
    // Дополнительная проверка: если указан конкретный формат с направлением - это не общий запрос
    if ((lower.includes('очн') || lower.includes('онлайн')) &&
...
export function isAskingForDetails(message: string): boolean {
  ...
  // Выбор словами
  const choiceWords = ['первый', 'второй', 'третий', 'первый курс', 'второй курс', 'третий курс'];
  ...
  if (message.length < 50 && courseNames.some(name => lower.includes(name))) {
    ...
    const questionWords = ['что', 'какой', 'какая', 'расскажи', 'про', 'подробнее'];
...
export function getShortCoursePresentation(courses: any[], qualification: ClientQualification): string {
  if (courses.length === 0) {
  ...
  if (qualification.direction === 'haircuts' && qualification.format === 'offline') {
    ...
    response += '• Фундамент — структурируем знания и раскладываем по полочкам\n';
