...
export const courseDates: Record<string, CourseDate[]> = {
  ...
  'Стрижки 2.0': [
    { date: '21-23 октября', format: 'offline' }
  ],
  'Планирование в салоне': [
    { date: '2-4 декабря', format: 'offline' }
  ]
};
...
export function getCourseDates(courseName: string): string {
  ...
  for (const [key, dates] of Object.entries(courseDates)) {
    if (courseName.toLowerCase().includes(key.toLowerCase()) ||
      ...
      if (courseName.toLowerCase().includes('очн') ||
      ...
      if (offlineDates.length > 0 && !courseName.toLowerCase().includes('онлайн')) {
        response = `🏡 **Ближайшие даты в Творческой деревне:**\n`;
        response += offlineDates.map(d => `• ${d.date}`).join('\n');
        response += '\n\nГруппы небольшие — максимум 15 человек. Формат "всё включено": проживание, питание, материалы.';
      }
      ...
      if (onlineDates.length > 0 && (response === '' || courseName.toLowerCase().includes('онлайн'))) {
        if (response !== '') response += '\n\n';
        response += `💻 **Ближайшие старты онлайн:**\n`;
        response += onlineDates.slice(0, 3).map(d => `• ${d.date}`).join('\n');
