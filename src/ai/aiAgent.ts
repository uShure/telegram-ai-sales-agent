...
import { COURSE_RULES, isAskingAboutInstallment, getInstallmentResponse, isAskingAboutProgram, getDetailedProgram, identifyCourseFromContext } from '../data/course-rules';
...
⚠️ ВАЖНЫЕ ПРАВИЛА О КУРСАХ:

ОЧНЫЕ КУРСЫ:
• Инструменты (ножницы, фен) НЕ ДАЕМ - привозите свои
• Записей очных курсов НЕТ - только живое обучение
• После очного курса НЕТ обратной связи - всё даётся на курсе
• Мужей на проживание НЕ БЕРЕМ - только участники
• За ночи НЕТ доплат - всё включено в стоимость

ОНЛАЙН КУРСЫ:
• Доступ к урокам - НА ГОД
• Если не успели сдать домашку вовремя - продление 3000₽/месяц
• Можно смотреть нескольким, но домашку проверяют только у оплатившего
• Есть РАССРОЧКА от академии (не банк!) на 2-6 месяцев без %

РАЗЛИЧИЯ:
• ТРЕНЕР - преподаватель, обучает других мастеров
• НАСТАВНИК - куратор онлайн-курса, проверяет домашки
• "Наставник по стрижкам" - практика на манекенах в ТРИ круга

ПРОГРАММЫ:
• ВСЕГДА расписывай программу курса в сообщении
• НЕ отправляй на сайт за программой
• Давай конкретные модули и темы
...
• Всегда уточняй формат обучения (очный/онлайн) - это критически важно!`;
  ...
  async generateResponse(
    ...
    try {
      ...
      // ПРИОРИТЕТ 2.5: Вопрос о рассрочке
      if (isAskingAboutInstallment(newMessage)) {
        console.log('💳 Вопрос о рассрочке');

        // Определяем, про какой тип курса спрашивают
        const allMessages = [...conversation.messages.map(m => m.content), newMessage];
        const context = allMessages.slice(-5).join(' ').toLowerCase();

        // Проверяем, упоминается ли офлайн/очный курс
        const isOfflineCourse = context.includes('очн') || context.includes('офлайн') ||
                                context.includes('творческ') || context.includes('деревн');

        const response = getInstallmentResponse(!isOfflineCourse);
      ...
      // ПРИОРИТЕТ 2.6: Запрос программы курса
      if (isAskingAboutProgram(newMessage)) {
        console.log('📚 Запрос программы курса');

        const allMessages = [...conversation.messages.map(m => m.content), newMessage];
        const courseName = identifyCourseFromContext(allMessages);

        if (courseName) {
          const program = getDetailedProgram(courseName);
          return {
            response: program,
            suggestedProducts: [],
            shouldScheduleFollowUp: true,
        ...
        // Если курс не определен, продолжаем обычную обработку
      }
